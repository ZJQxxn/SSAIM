...
epsilon_list = c(0.0, 0.25, 0.5, 0.75, 1.0)
lambda = 0.08
delta = 0.1
...
for (epsilon in epsilon_list){
    print(sprintf('========= %f Epsilon ============', epsilon))
    ...
    write.csv(t(estimatedTheta), sprintf('./estimation/SSAIM_%dp_%dn_%.3fepsilon_theta.csv', p, n, epsilon), row.names = FALSE)
    write.csv(estimate_F, sprintf('./estimation/SSAIM_%dp_%dn_%.3fepsilon_F.csv', p, n ,epsilon), row.names = FALSE)
    write.csv(as.matrix(estimate_F %*% t(estimatedTheta)), sprintf('./estimation/SSAIM_%dp_%dn_%.3fepsilon_Y.csv', p, n, epsilon), row.names = FALSE)
}
# Save to csv file
